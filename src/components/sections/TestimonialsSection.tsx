  ...
  return (
    <figure
      className={`flex h-full w-64 shrink-0 flex-col rounded-[28px] p-6 sm:w-72 md:w-auto md:p-7 ${
        dark ? 'bg-[#0C0C0C] text-white' : 'bg-[#F4F2ED] text-black/80'
      }`}
    >
      ...
      <figcaption className="mt-6 flex items-center gap-3">
        {testimonial.avatar ? (
          ...
          >
            <User size={18} className={dark ? 'text-white/60' : 'text-black/40'} />
          </span>
        )}
        <div>
          <p className="font-poppins text-sm font-semibold uppercase tracking-wide">{testimonial.name}</p>
          <p className={`text-sm ${dark ? 'text-white/60' : 'text-text-body'}`}>{testimonial.handle}</p>
        </div>
      </figcaption>
    </figure>
  );
  ...
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="mx-auto max-w-4xl px-4 text-center">
        <p className="font-sfpro text-sm uppercase tracking-wide text-text-body">Why the use us</p>
        <h2 className="mt-4 font-poppins text-4xl font-semibold text-black/80 md:text-5xl">
          Trusted by the best brands
        </h2>
        <p className="mx-auto mt-4 max-w-xl font-sfpro text-text-body">
