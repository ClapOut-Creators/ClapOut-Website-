...
export function abbreviateMoney(currency: string, amount: number | null | undefined): string {
  if (amount === null || amount === undefined || !Number.isFinite(amount)) return NOT_ANNOUNCED;
  const sign = amount < 0 ? '-' : '';
 ...
 */
function formatMoneyExact(currency: string, amount: number | null | undefined): string {
...
function budgetPercent(spent: number | null | undefined, total: number | null | undefined): number {
 ...
 */
export default function PublicCampaignCard({ campaign }: PublicCampaignCardProps) {
  ...
  const participantsLabel =
    campaign.registrationCount === undefined || campaign.registrationCount === null
      ? NOT_ANNOUNCED
            ...
            {campaign.platforms.map((platform) => (
              ...
              >
                <svg
                  ...
                  focusable="false"
                >
                  <path d={PLATFORM_GLYPH_PATHS[platform]} fill="currentColor" />
                </svg>
                <span className="sr-only">{PLATFORM_GLYPH_LABELS[platform]}</span>
              </span>
            ))}
          </div>
        )}
        ...
        <div className="mt-auto px-[14px] pt-[16.4px] xl:pt-[15.5px]">
          ...
          >
            <div
              ...
              style={{ width: `${budgetPercent(campaign.budgetSpent, campaign.budgetTotal)}%` }}
            />
          </div>
        </div>
      )}
